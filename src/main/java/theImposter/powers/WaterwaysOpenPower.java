...
public class WaterwaysOpenPower extends AbstractPower  implements BetterOnApplyPowerPower {
    private AbstractCreature source;
    public static final String POWER_NAME = "Waterways Open";
    public static final String POWER_ID = makeID(POWER_NAME.replaceAll("([ ])", ""));
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;

    public WaterwaysOpenPower(AbstractCreature owner, int amount) {
    ...
    public int betterOnApplyPowerStacks(AbstractPower power, AbstractCreature target, AbstractCreature source, int stackAmount)
    {
        if (power.ID.equals(SusPower.POWER_ID)) {
            power.amount += this.owner.getPower(this.POWER_ID).amount;
            stackAmount += this.owner.getPower(this.POWER_ID).amount;
        }
        return stackAmount;
    }
